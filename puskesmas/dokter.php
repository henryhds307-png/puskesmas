...
if (!function_exists("GetSQLValueString")) {
  ...
  {
    // Tidak perlu cek magic quotes (sudah tidak ada di PHP modern)
    global $koneksi;
...
// Query utama
$query_jadwal = "SELECT * FROM jdwl_dokter 
INNER JOIN v_dokter ON jdwl_dokter.kd_dokter = v_dokter.kd_dokter";
$query_limit_jadwal = sprintf("%s LIMIT %d, %d", $query_jadwal, $startRow_jadwal, $maxRows_jadwal);

// Eksekusi query dengan MySQLi
$jadwal = mysqli_query($koneksi, $query_limit_jadwal) or die(mysqli_error($koneksi));
...
if (isset($_GET['totalRows_jadwal'])) {
  ...
  $all_jadwal = mysqli_query($koneksi, $query_jadwal);
  $totalRows_jadwal = mysqli_num_rows($all_jadwal);
}
...
<link rel='stylesheet' href='https://cdn.datatables.net/1.10.22/css/jquery.dataTables.min.css'/>
    ...
    <tbody>
                <tr>
                    <td><?php echo $no++; ?></td>
                            <td><?php echo $row_jadwal['nip_dokter']; ?></td>
                            <td><?php echo $row_jadwal['nm_dokter']; ?></td>
                            <td><?php echo $row_jadwal['no_hp']; ?></td>
                            <td><?php echo $row_jadwal['spesialis']; ?></td>
                            <td><?php echo $row_jadwal['nm_poli']; ?></td>
                            <td><?php echo $row_jadwal['hari']; ?></td>
                            <td><?php echo $row_jadwal['jam_awal']; ?> s.d <?php echo $row_jadwal['jam_akhir']; ?></td>
                          </tr>
                          <?php } while ($row_jadwal = mysqli_fetch_assoc($jadwal)); ?>
            </tbody>
                      </table>
                    </div>
				</div>
