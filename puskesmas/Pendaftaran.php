...
if (!function_exists("GetSQLValueString")) {
    function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "")
    {
        ...
        // Kompabilitas PHP lama (hapus magic quotes)
        if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) {
            $theValue = stripslashes($theValue);
        }

        // Escape string (mysqli / mysql / fallback)
        if (function_exists("mysqli_real_escape_string") && isset($koneksi)) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    $insertSQL = sprintf(
        "INSERT INTO pasien (nm_pasien, jenkel, tmpt_lahir, tgl_lahir, alamat, kd_poli, nm_dokter, tgl_berobat, keluhan, no_hp)
